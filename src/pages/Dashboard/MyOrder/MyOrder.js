...
const MyOrder = () => {
  ...
  useEffect(() => {
    fetch(`https://car-service-psi.vercel.app/orders?email=${user.email}`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
      });
  ...
  const handleCancel = (id) => {
    ...
    if (proceed) {
      fetch(`https://car-service-psi.vercel.app/deleteOrder/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <div>
      <h2>
        Ordered List of{" "}
        <span style={{ color: "blue" }}>{user.displayName}</span>
      </h2>
                ...
                <TableCell align="center">
                  {row?.status === "Pending" ? (
                    <Box>
                      <Typography
                        variant="body1"
                        sx={{ color: "goldenrod", display: "inline", p: "5px" }}
                      >
                        {row?.status}
                      </Typography>
                      <Button
